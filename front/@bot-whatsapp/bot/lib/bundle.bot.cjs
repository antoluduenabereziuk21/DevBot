...
//todo transpilado para detectar cuando te envian un contacto
const { generateRef: generateRef$10 } = hash;

const eventContact$1 = () => {
    return generateRef$10('_event_contact_')
};

const REGEX_EVENT_CONTACT$1 = /^_event_contact__[\w\d]{8}-(?:[\w\d]{4}-){3}[\w\d]{12}$/;

var eventContact_1 = { eventContact: eventContact$1, REGEX_EVENT_CONTACT: REGEX_EVENT_CONTACT$1 };
...
const { eventContact, REGEX_EVENT_CONTACT } = eventContact_1;
...
let CoreClass$1 = class CoreClass extends EventEmitter$1 {
    ...
    handleMsg = async (messageCtxInComming) => {
        ...
        if (!prevMsg?.options?.capture) {
            ...
            //todo agregar evento de contacto
